...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        sh "helm upgrade --dry-run --install ${args.name} ${args.chart_dir} --set imageTag=${args.imageTag},replicas=${args.replicas},cpu=${args.cpu},memory=${args.memory},ingress.hostname=${args.hostname} --namespace=${namespace}"
    } else {
        println "Running deployment"

        // reimplement --wait once it works reliable
        sh "helm upgrade --install ${args.name} ${args.chart_dir} --set imageTag=${args.imageTag},replicas=${args.replicas},cpu=${args.cpu},memory=${args.memory},ingress.hostname=${args.hostname} --namespace=${namespace}"
...
def containerBuildPub(Map args) {
    ...
    // docker.withRegistry("https://${args.host}", "${args.auth_id}") {
        ...
        //}
        sh "docker tag ${args.acct}/${args.repo} ${args.acct}/${args.repo}:${env.GIT_SHA}"
        sh "docker push ${args.acct}/${args.repo}:${env.GIT_SHA}"
